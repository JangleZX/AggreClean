...
class CasualLLMVictim(Victim):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.llm: Union[LlamaForCausalLM, MptForCausalLM, GPT2LMHeadModel] = AutoModelForCausalLM.from_pretrained(path, config=self.model_config, trust_remote_code=True, device_map="auto",attn_implementation="flash_attention_2",torch_dtype='auto')
        print(self.llm)
    ...
    def transfer2Muscle(self):  
        if self.muscleConfig.get('lora') and self.muscleConfig.get('loraConfig') is not None:
            loraConfig = LoraConfig(**self.muscleConfig.get('loraConfig'), task_type=TaskType.CAUSAL_LM)
            mixed = self.muscleConfig.get('mslr') is not None
            # self.llm = get_peft_model(self.llm.base_model, loraConfig, mixed=mixed, adapter_name='lora')
            # self.llm = get_peft_model(self.llm.base_model, loraConfig)
    ...
    def trainProcess(self, batch):
        ...
        #         print(f"发现无效target: {target} (类型: {type(target)})")
        #         target = "[EMPTY]"  # 添加默认值
        #     targetIds.append(self.tokenizer.encode(target, ...))
    ...
    def _tunable_parameters_names(self, module: Optional[nn.Module]=None):
        ...
        gradPara =  [n for n, p in module.named_parameters() if p.requires_grad and ('lm_head' not in n)]
    ...
    def save(self, ckpt_path=None, config=None):
