...
class CasualTrainer(Trainer):
    ...
    def train_one_epoch(self, epoch: int, epoch_iterator):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss = output.loss
            if torch.isnan(loss):
                print(f"[ERROR] Epoch {epoch} Step {step} 计算出 NaN！")
                print(" batch_labels 样本前 10 个：", batch_labels.view(-1)[:10])
                print(" batch_inputs IDs 前 1 行：", batch_inputs[0][:10])
                return float('nan'), 0, 0  # 或者 raise RuntimeError
