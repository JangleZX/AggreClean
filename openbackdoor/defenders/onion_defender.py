...
class ONIONDefender(Defender):
    ...
    def correct(
        ...
        for (poison_text, label, poison_label) in poison_data:
            if len(poison_text.split()) > 1:
                ...
                if process_text != poison_text:
                    logger.info(
                        f"[ONION Defense - Client {client_id}] 检测到有毒样本并修改:\n"
                        f"  原始文本: {poison_text}\n"
                        f"  净化文本: {process_text}"
                    )
        ...
        logger.info(f"finish onion defend")
        print('\n'*2)
        logger.info(f"process_data_li type: {type(process_data_li)}")
