...
def add_gaussian_noise(t, clip_norm=1.0, eps=4.0, delta=1e-5, device=None):
    """
    ...
    C = clip_norm
    # Rényi accountant => σ ≈ C * √(2 ln(1.25/δ)) / ε
    sigma = C * math.sqrt(2.0 * math.log(1.25 / delta)) / eps
    noise = torch.randn_like(t, device=device) * sigma
    return t + noise
...
class GraCeFulDefender(Defender):
    ...
    """
    def __init__(self, targetPara="lm_head.weight", targetDataset="webqa",
                 pcaRank=32, dp_eps=4.0, dp_delta=1e-5, clip_norm=1.0,noise_std=0.5, **kwargs):
    ...
    def correct(
        ...
        embed, poisonLabels = self.encode(poison_data, model)
        # print("oriembed:",embed)
        # print("lastembed:",embeddings)
        # print("oriembedsize:",embed.shape)
        # print("lastembedsize:",embeddings.shape)

        if client_preds is not None:
            predLabels=client_preds
            logger.info(f">> ==================== using global preds ====================")
        else:
            # Step 2. Hierarchical Clustering
            predLabels = self.clustering(embed,centers)
        ...
        embUmap = umap.fit(embed).embedding_
        ...
        try:
            ...
            logging.warning(f"Silhouette 计算失败: {e}. 返回 None。")
            logger.info(f'silhouette score of {client_id}: 0')
    ...
    def encode_encrypt(self, dataset, model,fl=None):
        dataloader = getCasualDataloader(dataset, batch_size=1, shuffle=False)
        dctGrads, poisonLabels = self.computeGradients(model, dataloader, "train")
        # dctGrads=l2_normalize_rows_tensor(dctGrads)
        logger.info("Reducing the dimension of hidden states")
        ...
        if fl is not None:
            # to NumPy for easier math
            grads = dctGrads.cpu().numpy()

            μ  = grads.mean(axis=0, keepdims=True)
            σ  = grads.std(axis=0, keepdims=True) + 1e-8        # avoid ÷0
            z  = (grads - μ) / σ                                # standardize
            z += np.random.normal(0.0, self.noise_std, z.shape) # add noise
            logger.info(f"self.noise_std: {self.noise_std}")
            grads_enc = z * σ + μ                               # inverse-transform

            return embeddings, poisonLabels, grads_enc
    ...
    #     clip = self.clip_norm
    #     l2norm = dctGrads.norm(p=2, dim=1, keepdim=True).clamp_min(1e-12)
    #     dctGrads = dctGrads * (clip / l2norm).clamp(max=1.0)
    #     dctGrads = add_gaussian_noise(dctGrads, clip_norm=clip,
    #                                   eps=self.dp_eps,
    #                                   delta=self.dp_delta)
    #     # dctGrads=l2_normalize_rows_tensor(dctGrads)
    #     logger.info("Reducing the dimension of hidden states")
    ...
    # def clustering(self, embeddings, centers=None, metric="cosine"):
    #     """
    #     K-Means-based clustering.
    #     Args:
    #         embeddings (np.ndarray)  : shape (n, dim)
    #         centers (np.ndarray|None): optional global centers, shape (2, dim)
    #         metric (str)             : 'cosine' or 'euclidean'
    #     Returns:
    #         If centers is None:
    #             predLabels (n,) , new_centers (2, dim)
    #         Else:
    #             predLabels (n,)         # same API as原函数
    #     """
    ...
    #     # === 带 / 不带 global centers 都用 KMeans ===
    #     if centers is not None:
    #         kmeans = KMeans(n_clusters=2, init=centers, n_init=1, random_state=0)
    #         kmeans.fit(X)                      # 仅对 samples 拟合
    #         sample_labels = kmeans.labels_     # (n,)
    #         # 哪个簇包含更多 global centers?
    #         center_labels = kmeans.predict(centers)
    #         majority = np.bincount(center_labels).argmax()
    #         predLabels = np.where(sample_labels == majority, 0, 1)
    #         ret = np.asarray(predLabels)
    #     else:
    #         kmeans = KMeans(n_clusters=2, random_state=0)
    #         kmeans.fit(X)
    #         clusterLabels = kmeans.labels_
    #         majority = np.bincount(clusterLabels).argmax()
    #         predLabels = np.where(clusterLabels == majority, 0, 1)
    #         ret = (np.asarray(predLabels), kmeans.cluster_centers_)

    #     logger.info(f"[Timing] k-means clustering took {time.perf_counter()-start:.4f}s")
    #     return ret
    ...
    def clustering(self, embeddings, centers=None, metric="cosine"):
        ...
        if metric == "cosine":
            X = normalize(X, axis=1)
            metric_used = "euclidean"          # 归一化后欧氏≃余弦
            if centers is not None:
                centers = normalize(centers, axis=1)
        else:
        ...
        # ============= ① 统一做 HDBSCAN =============
        if centers is not None:
            data = np.vstack([X, centers])     # samples + global centers
            clusterer = hdbscan.HDBSCAN(min_cluster_size=max(5, len(data)//30),
                                        metric=metric_used)
            labels_all = clusterer.fit_predict(data)

            sample_labels  = labels_all[:len(X)]
            center_labels  = labels_all[len(X):]

            # 选中心最多的簇为“正常簇”（忽略 -1 噪声）
            valid_center_labels = center_labels[center_labels != -1]
            if valid_center_labels.size:
                majority = np.bincount(valid_center_labels).argmax()
            else:                              # 所有中心被判噪声→取样本最大簇兜底
                majority = np.bincount(sample_labels[sample_labels != -1]).argmax()

            predLabels = np.where(sample_labels == majority, 0, 1)
            predLabels[sample_labels == -1] = 1                 # 噪声必为异常
            predLabels = 1 - predLabels    
            logger.info(f"[Timing] HDBSCAN (+centers) {time.perf_counter()-tic:.4f}s")
            return predLabels.astype(int)

        else:
            clusterer = hdbscan.HDBSCAN(min_cluster_size=max(5, len(X)//30),
                                        metric=metric_used)
            clusterLabels = clusterer.fit_predict(X)             # -1 = noise
            valid = clusterLabels != -1
            if valid.any():
                majority = np.bincount(clusterLabels[valid]).argmax()
            else:
                majority = -1                                   # 退化：全噪声

            predLabels = np.where(clusterLabels == majority, 0, 1)
            predLabels[clusterLabels == -1] = 1
            predLabels = 1 - predLabels
            # 计算每个有效簇质心（忽略噪声）
            centers_list = []
            for lbl in np.unique(clusterLabels):
                if lbl == -1: continue
                centers_list.append(X[clusterLabels == lbl].mean(axis=0))
            cluster_centers = np.vstack(centers_list) if centers_list else np.empty((0, X.shape[1]))

            logger.info(f"[Timing] HDBSCAN {time.perf_counter()-tic:.4f}s")
            return predLabels.astype(int), cluster_centers

    # def clustering(self, embeddings,centers=None, metric="cosine", linkage_method='average'):
    #     if centers is not None:
    #         start = time.perf_counter()

    #         # 1) 先合并：把 global centers 也加到样本里
    #         data = np.vstack([embeddings, centers])         # shape (n+2, dim)
    #         # 2) 计算两两距离矩阵
    #         pdist_mat = pdist(data, metric=metric)          # condensed distance matrix
    #         # 3) 做层次聚类
    #         Z = linkage(pdist_mat, method=linkage_method)
    #         # 4) 切成 2 个簇（包括 samples+centers）
    #         labels_all = fcluster(Z, t=2, criterion='maxclust') - 1  # 变成 [0,1]
    #         sample_labels = labels_all[:embeddings.shape[0]]
    #         center_labels = labels_all[embeddings.shape[0]:]         # 中心的簇标签

    #         # 5) 统计哪个簇里包含了 majority 的 global centers
    #         #    假设正常是大多数中心那一簇
    #         unique, counts = np.unique(center_labels, return_counts=True)
    #         labelCounts = dict(zip(unique, counts))
    #         majority_center_cluster = max(labelCounts, key=labelCounts.get)

    #         # 6) 生成 predLabels：和中心同簇视为 0，反之视为 1
    #         predLabels = np.where(sample_labels == majority_center_cluster, 0, 1)

    #         elapsed = time.perf_counter() - start
    #         print(f"[Timing] hierarchical clustering + labeling took {elapsed:.6f} seconds")
    #         return np.array(predLabels)
    #     if centers is None:
    #         logger.info("Clustering the low dimensional embeddings")
    #         clusting = AgglomerativeClustering(n_clusters=2, metric=metric, linkage=linkage_method) # 用层次聚类把样本划分为两类、用余弦距离来衡量样本相似性、簇间距离计算采用​​平均链接法
    ...
    #         return np.array(predLabels), np.vstack(cluster_centers) 
    ...
    def dimensionReduction(
        ...
        _, _, V = torch.pca_lowrank(hiddenStates, q=min(pcaRank, hiddenStates.shape[0], hiddenStates.shape[1]), center=True)
